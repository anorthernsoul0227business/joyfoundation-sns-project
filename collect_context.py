...
# 2026-09-01、三度目の誤作動。「…津波警報・噴火警戒レベル4以上…は**確認されなかった**」
# を災害発生と読んだ。語を一つずつ足す方式では、平常時の言い回しが少し変わるたびに
# 同じことが起きる。そこで**否定の語尾**そのものを捉える。
#
# 日本語の否定はほぼ「ず・ぬ・ない・なく・なかった・ません・なし」で終わる。
# 節の末尾がこの形なら、その節に並ぶ危険語は「無かったこと」の列挙とみなす。
# 判定は節ごとなので、「震度6弱が発生し、けが人はいない」のような文では
# 前半の節が否定で終わらず、キルスイッチは正しく作動する。
_NEG_END = r"(?:ず|ぬ|ない|なく|なかった|ません|ませんでした|なし)(?:[。、）\)」』]|\s|$)"
...
def fetch(today: date, timeout: int = 600, retries: int = 3) -> dict:
    ...
    for attempt in range(1, retries + 1):
        try:
            # --allowedTools を明示する理由:
            #   この工程は WebSearch が使えないと成立しない。許可が無いと
            #   claude は「権限が許可されていないため実行できません」と
            #   answering してしまい、JSON が返らず3回とも失敗する
            #   （2026-09-01、Mac mini へ移した初回に踏んだ）。
            #
            #   MacBook では settings.json の defaultMode:"auto" で全ツールが
            #   通っていたため露見しなかった。実行機の設定に依存させず、
            #   必要な1つだけをコード側で指定する。
            proc = subprocess.run(
