...
## Testimonial カード（`knowledge/testimonial/TM-XXXX.md`）

体験された方ご本人の言葉。**Evidence ではない。**

数値は「何が起きたか」を示すが、参加された方が実際にどう感じたかは残らない。
その層をここで持つ。ただし**個人の感想を効果の証拠として提示してはいけない**
（薬機法・景品表示法）。
...
source_type: 被検者の感想
collected: 2024-01
related_evidence: [EV-0013, EV-0014]   # 同じ試験の定量データ

anonymized: true            # 実名は必ず伏せる
subject_label: 参加者A       # 通し記号のみ。氏名・年齢・職業は書かない
condition: 自然音 | 都会音   # どちらの条件での感想か

verbatim: |
  （ご本人の言葉のまま。氏名と個人的事情の注記は除く）
...
- **氏名を書かない。** 原本には実名（一部はカナ読み）があるが、カードには入れない
- **個人的な事情の注記を入れない。** 原本の「＊参考：」欄には健康状態や
  家庭の事情が書かれている。これは記録用であり、カードにも記事にも入れない
- **効果の証拠として提示しない。** 引用するときは「個人の感想です」を添える
- 掲載の可否は 2026-08-04 に「公開可」の判断を得ているが、
  それは**実名のまま出してよいという意味ではない**

### 出典ファイルの扱い

原本（`docs/sources/nb1_04〜06`）は実名と健康状態を含むため `.gitignore` 済み。
カードは匿名化してあるのでコミットしてよい。
