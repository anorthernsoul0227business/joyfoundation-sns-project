...
## 13. 未決定事項（Codex壁打ちで決めたい）

1. **FullCalendar vs react-big-calendar vs 自作**: D&Dとの統合品質、ライセンスコスト
2. **Supabase vs 独自Auth + 独自DB**: Supabaseロックインのリスク vs 開発速度
3. **Celery vs BullMQ vs Supabase Edge Functions**: ジョブスケジューラの選択
4. **モノレポ vs 分離リポ**: frontend / backend の管理方法
5. **note.com対応方法**: 公式APIが限定的。スクレイピング vs 手動 vs API待ち
6. **画像自動変換のタイミング**: アップロード時 vs 投稿直前
7. **オフライン対応（アプリ化時）**: PWA Service Worker でどこまで対応するか
8. **AI APIの選定**: Claude API vs OpenAI API vs 両方対応。コスト・品質・日本語性能の比較
9. **プロンプト管理**: ハードコード vs DB保存 vs ファイルテンプレート。ユーザーごとのカス���マイズ対応
10. **音声文字起こし**: Whisper API vs Google Speech-to-Text vs ローカルWhisper。コストと精度
11. **生成結果のキャッシュ**: 同じ資料で再生成した場合の差分管理方法
