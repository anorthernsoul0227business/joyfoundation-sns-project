...
## 2026-09-02（朝・共有ボード画面の初版が動いた／iCloud の「中身なしファイル」が原因でツールが固まっていた）

### できたこと
- `/board` 画面の初版を実装し、`tsc` `eslint` `next build` すべて通過。
  - 新規: `apps/web/src/app/board/page.tsx`、`components/board/{ArticleList,ArticleDetail,StatusBadge,FontSizeControl,IdeaDock,SidePanels}.tsx`、`lib/{auth,board}.ts`
  - 変更: `lib/supabase.ts`（セッション永続化）、`app/login/page.tsx`（FastAPI → supabase-js 認証）、`components/AppShell.tsx`（/board では共通ヘッダーを出さない）、`components/AppHeader.tsx`（ログアウトを supabase-js に）
  - 圭一郎さんの操作は「このまま出す」「直したいところがある」の2択のみ。文字サイズ 3段階は html の font-size に反映し端末に記憶
- Mac mini 上で本番ビルドを起動し Tailscale 経由で確認可能: `http://100.78.122.50:3000/login`

### 判明したこと（重要）
- `Desktop/AI関連/joyfoundation_project` は iCloud Drive 配下で、ファイルの **98% が「dataless」（中身は iCloud 側にあり、読むたびに 1 ファイル約 1.2 秒かけて取得）** だった。
  - `node_modules` 24,239 ファイル中 23,733、ソース 5,839 ファイル、`.git` 2,797 ファイルが該当
  - これが昨日からの `git status` 2 分超、`tsc` 5 分無応答の正体。ツールの問題ではない
- 対処: iCloud の外に作業用クローンを置いた → `~/dev/joyfoundation-sns-project`（origin から clone + ローカル 2 コミットを bundle で移送 + 今日の未コミット差分を rsync）。ここでは `pnpm install` 25 秒、`tsc` 2.5 秒。
  - **編集の正本は引き続き iCloud 側**。クローンはビルド・検証用で、`rsync --files-from` で差分を写している
  - `brctl download` で iCloud 側の取得も要求済み（進行は遅い）
- ローカルクローンに `.gitignore` 対象の `apps/web/.env.local` と `src/generated/` を手で複製した（build に必要）

### まだのこと
- 実 Supabase へのマイグレーション適用（DB パスワード or CLI アクセストークン待ち）。適用前は `/board` の一覧が「relation articles does not exist」で空になる
- 既存アカウントでログインできるか（Supabase Auth 側のユーザー有無）は康二郎さんに実機で確認してもらう
- `run_weekly_loop.py` の `publish()` / `load_recent()` の Supabase 移行

### 追記（09-02 午後・修正の自動反映、投稿日の自動決定、カレンダー連携）

**圭一郎さんのアカウントを作成**（kita@h-garden.com / role=editor）
- 新規登録トリガーが専用組織を自動作成するため、康二郎さんの組織へ付け替えて空組織を削除
- editor なので記事60件が見えるが削除はできない（RLSで確認済み）

**修正依頼を AI が反映する仕組み**（`apply_fixes.py`）
- `needs_fix` の記事を拾い、指示どおり本文を直して `revised`（＝直しました）にする
- 「指示された箇所以外は一字も変えない」「曖昧なら直さず保留」を最優先の制約に
- 実データ9件で検証 → 反映6件・保留3件。保留は画像差し替え依頼やURL未指定など、
  AI が推測で埋めずに聞き返したもの。狙いどおり
- **字数超過への対処**: 指示どおり直すと X が141字になった。案A（黙って縮める）ではなく
  **案B＋提案**（「『約15分』を『15分』にしました。よろしいですか」と聞く）を採用。
  `articles.revision_note` に申し送りを入れ、承諾されるまで投稿しない

**画面に差分表示**（`BodyDiff.tsx`）
- はじめ文字単位で差分を取ったが読みにくく、**文単位**に変更（康二郎さんの指摘）
- さらに IG は一文の途中で改行が入るため、改行では切らず句点と空行だけで区切るよう修正。
  ART-0047 の変更箇所が 29→11 に整理された

**承認→投稿の橋渡し**（`schedule_posts.py`、launchd 毎朝8:00）
- **投稿が 6/16 から止まっていた**。承認と投稿キューの間を人が繋いでいて途絶えていた
- イベント記事は開催日から逆算（既定3日前）。間に合わなければ投稿せず `missed` にする
- 期日なしは翌日から1日1件。同じ媒体で1日1件、同じイベントを同日に重ねない
- **note には自動投稿の仕組みが無い**（X/IGのみ）。日時だけ決めて手投稿に回す

**Google カレンダーを開催日の正本に**（`sync_events.py`）
- 圭一郎さんのカレンダー（keiichiro.kita@gmail.com）を events に取り込み（29件）
- シートの23件より新しく、11〜12月の予定5件と 9/6 追加公演が増えた
- 費用はカレンダーに無いためシートから補う。表記揺れ（ウエ/ウェ、空白）で
  先頭一致が効かず、日付を軸にした類似度照合に変更
- `events.lead_days` を追加。イベントごとに「何日前に告知するか」を変えられる

**直したバグ**
- 取り込み時に列I「修正版」を body_final に入れていた。実際には圭一郎さんが
  「こう直して」という**指示**を書く欄で、ボードが本文の代わりに指示文を表示していた
- 改行が `\n` の文字列のまま保存されていた（csv の escapechar 二重化）
- 記事番号 ART-0040 の重複 → DBシーケンスで採番するよう変更
- 終日予定が UTC 変換で1日ずれ、9/11 のイベント記事3件が「期日なし」と誤判定
- Calendar 接続で `keiichirou` と綴り誤り（正しくは `keiichiro`）。403→404 を
  共有漏れと誤診断した

**Vercel 本番公開**: https://shc-sns-calendar-web.vercel.app/board
- `src/generated` が .gitignore 対象でアップロードされないため、
  buildCommand に `generate:api-client` を追加（生成元 openapi.json は git 管理下）

### 追記（09-02 夜・週次ループを共有ボードへ接続）
- **記事の保存先を 週次_レビュー から Supabase の `articles` に移した**
  - 新規 `supabase_store.py`: PostgREST を標準ライブラリ（urllib）だけで叩く。
    launchd 上の依存を増やさないため psycopg は使わない。4回リトライ付き
  - `run_weekly_loop.py`: `publish()` は記事を Supabase へ、内訳は 週次_内訳 に残す
    （内訳は康二郎さん用の診断情報で、ボードには出さないため）。
    `load_recent()` も Supabase から読む
  - `republish.py`: `publish()` を呼ぶ前に `.env` を読むよう修正（Supabase 接続情報が要るため）
  - `.env` に `SUPABASE_URL` / `SUPABASE_SERVICE_ROLE_KEY` を追加（プロジェクト・実行環境の両方）
  - `sync_loop_runtime.sh` の SCRIPTS に `supabase_store.py` を追加
- **記事番号の重複バグを修正**（migration `20260902120000_article_no_sequence.sql`）
  - `article_no` の既定値を `public.next_article_no()`（シーケンス）に。行数から数えないので
    行を削除しても重複しない。現在値は 57 の次から
- **今朝の失敗ぶんを復旧**: 09:00 の定期実行が生成3本を作ったあと、④投入で
  `ConnectionResetError` によりシート書き込みに失敗し `unpublished_20260902_091431.json` に退避されていた。
  修正後の `republish.py` で **ART-0063〜0065（画像5枚つき）を共有ボードへ復旧**。記事は計60件に
  - 皮肉だが、シートへの書き込みが不安定だったこと自体が今回の移行の裏づけになった
- 検証: モジュール単体（読み・書き・添付）→ publish() 結線 → ループ dry-run → 実データ復旧、の順に確認。
  テスト行はすべて削除済み（DB・内訳シートとも）

**注意（要対応）**: `sns-calendar-app/.env` の `SUPABASE_SERVICE_ROLE_KEY` と
DBパスワードが**同一の値**だった（`sb_secret_…`）。片方を失効させると両方が止まる。
いずれ別々に発行し直すのが望ましい。

### 追記（09-02 夕・iPhone対応と改行バグ修正）
- **iPhone で枠がはみ出す問題を修正**。`13rem + 19rem` の2ペインを狭い画面では縦積みにし、
  一覧と本文を切り替え表示（「← 一覧にもどる」）に。基準文字サイズも iPhone では 14/16/18px
  （PCは 17/19/22px）。左ナビは上部の横並びタブへ。`useIsNarrow()` を新設
  - 康二郎さん確認済み「見やすくなりました」
- **改行が `\n` と表示されるバグを修正**。原因は取り込みスクリプトの二重エスケープで、
  `csv.DictWriter(quoting=QUOTE_NONE, escapechar="\\")` が**エスケープ文字自身も二重化**するため、
  改行が `\\n` → COPY text が `\n`（文字列）として格納していた。
  - 対処: csv モジュールをやめ `copy_escape()` / `write_tsv()` を自作。既存57件も UPDATE で修復
  - 検証: バックスラッシュを含む行 0 件、実改行を持つ記事 54 件。再取り込みしても件数は 57 のまま
  - 教訓: LIKE のパターン内では `\` がエスケープ文字。`body like '%\n%'` は「文字 n」に一致してしまい、
    誤った検証結果（12件残存）を出した。文字の有無は `strpos(col, chr(92))` で確認すること
- 康二郎さんから今後の要望3件を受領 → `docs/共有ボード_今後の実装予定.md` に整理
  （①画像の直接アップロード ②投稿済みカレンダーと再投稿 ③思いつきメモにLLM）

### 追記（09-02 昼・本番 Supabase へ適用完了）
- 康二郎さんから DB パスワードを受領（Database Settings → Reset password で発行）。`~/.config/jf/supabase.env`（600）に保存
- `20260901190000_shared_board.sql` を **本番 Supabase に適用**。6テーブル・22ポリシー作成、`supabase_migrations.schema_migrations` にも記録（既存8件 → 9件）
- `import_review_sheet_to_supabase.py` を新規作成し、`週次_レビュー` の **記事57件・画像26件** を取り込み
  - ステータス内訳: ai_draft 48 / needs_check 4 / needs_fix 5
  - `TEST-` 接頭辞は除去（全記事が `--test-cards` 生成のため）
  - 「修正版」欄に書き込みがあり理由が空の5件は、制約を満たすため fix_note に定型文を入れた
  - 再実行しても増えない（`ON CONFLICT (org_id, article_no)`）。ボードで判断済み（`reviewed_at` あり）の行はシートの古い状態で上書きしない
- RLS 動作確認: `authenticated` ロール + 実ユーザーの sub で 57件・添付26件が見える
- **シートのバグを発見**: `ART-0040` が2件重複（8/17 note と 8/19 X）。`publish()` の採番が
  「既存行数」から計算しているため、行を削除すると同じ番号が再発行される。
  取り込みでは2件目を `ART-0040-2` として退避したが、**publish() 側の採番修正が必要**

## 2026-09-01（夕方・共有ボード着手：Cloud Run を外し Vercel + Supabase の2層へ）

### 背景

圭一郎さんとの情報共有が見にくいという相談から着手。`週次_レビュー` タブが18列あり
横スクロールが頻発、記事が増えるほど縦に伸びて特定できない。メール・LINE・NotebookLM・
スプレッドシートに窓口が分散している状態も併せて解消する。

### 決めたこと

| 論点 | 決定 |
|---|---|
| 土台 | 新規開発せず既存 `sns-calendar-app` を復活させる |
| 対象デバイス | PC。左=一覧・右=本文の2ペイン。**横スクロールは出さない** |
| 統合する4機能 | ①記事の承認 ②思いつきメモ ③イベントと画像 ④資料・情報の共有 |
| 圭一郎さんの操作 | **「このまま出す」「直したいところがある」の2択のみ**。修正種別13コードの
プルダウンは廃止し、自由記入にして分類は康二郎さんが後付けする |
| バックエンド | **Cloud Run を廃止し Vercel + Supabase の2層に**。ただし GCP プロジェクトは
削除せず停止のまま残す（停止中は費用ゼロ。将来の音声入力等で必要になれば戻せる） |
| 商用利用規約 | 考慮対象から外す（圭一郎さんと2人で使う前提） |

### 実測した既存アプリの状態

- フロント（Vercel）: **生存**。https://shc-sns-calendar-web.vercel.app が 200
- Supabase: **生存**。ref `msghvqclexpvgkrctxug` が応答（一時停止していない）
- **Cloud Run: 停止**。`/health` が HTTP 500
- **Mac mini に開発環境が無かった**。pnpm / vercel / supabase / gcloud すべて未導入、Node は v26（`.nvmrc` は 20）
- GCP 認証情報もこの機械には無い（`~/.config/gcloud` が存在しない）。唯一ある
  `~/.config/gspread/service_account.json` は**別プロジェクト** `gmailsendapp-478003` の
  Sheets/Drive 用サービスアカウントで、`shc-sns-calendar` には使えない
...
| 中身 | 行数 | 2層化後 |
|---|---|---|
| 記事・カレンダー・通知・認証の取次ぎ | 939 | 不要（ブラウザから Supabase を直接） |
| SNS投稿の実行・配信キュー | 1,176 | 不要（Mac mini の launchd ジョブと重複） |
| メール通知 | 218 | 不要（`~/agent/sns/notifier.py` と重複） |
| 画像アップロード・Drive ピッカー | 653 | **TypeScript へ移植が必要** |
| X/IG の OAuth 連携 | 666 | 破棄（Mac mini は `.env` のトークンで投稿しており未使用） |

実際に書き直すのは画像まわりの653行のみ。Vercel Hobby の関数実行時間（既定10秒）に
Pillow 処理が当たる懸念は、**ブラウザ側で先に縮小してから送る**設計で回避する。
なお自動投稿は Mac mini が R2 の公開URLを直接読むため、Vercel が時間切れになっても
投稿は止まらない。

### 実装したもの

**`supabase/migrations/20260901190000_shared_board.sql`（341行）**

| テーブル | 役割 | 後継元 |
|---|---|---|
| `articles` | 記事のレビュー・承認 | `週次_レビュー` タブ |
| `article_reviews` | 承認・修正依頼の履歴 | （新規。シートは上書きで履歴が消えていた） |
| `ideas` | 思いつきメモ | メール・LINE |
| `events` | イベント情報 | `イベント予定` タブ |
| `shares` | 資料・お知らせ・質問 | メール・NotebookLM |
| `attachments` | 画像添付（4種の親に付く） | Drive + シートの画像列 |

既存規約（`get_user_org_ids()` / `can_edit_org()` / `set_updated_at()`）に合わせた。
`body_ai` と `body_final` を分け、AI原稿を上書きさせない方針はシートから引き継いでいる。

**`docs/mockup_共有ボード_v1.html`** — 圭一郎さん向け画面のモック。文字サイズ3段階、
未対応3件のみ初期表示、補足情報は折りたたみ、右下に常駐する思いつきメモ入口。

### 検証

Mac mini に PostgreSQL 16 / Node 20 / pnpm を導入（`brew`）。Docker が無く supabase CLI の
ローカル環境が使えないため、**素の PostgreSQL 16 に Supabase 相当のスタブ**（`auth.users`・
`auth.uid()`・`anon`/`authenticated`/`service_role`/`postgres` ロール）を作って検証した。

- 既存マイグレーション6本 → 全て適用OK
- 新規 `20260901190000_shared_board.sql` → 適用OK
- 制約テスト7件すべて期待どおり弾いた（修正内容が空の `needs_fix`、投稿日時の無い
  `published`、不正な status、`article_no` 重複、終了が開始より前のイベント、
  確認日時の無い確認済みイベント、不正な `owner_type`）
- RLS 6テーブルすべて有効、ポリシー22件、`updated_at` トリガ動作を確認

検証用DB `jf_verify` は残してある（`pg_ctl -D /opt/homebrew/var/postgresql@16 start`）。

### 訂正

シートへの依存を「2箇所だけ」と報告したが**誤り**だった。実行中のコードを数え直すと
4ファイル10箇所ある。

```
loop/run_weekly_loop.py  311, 423, 461, 1265, 1266
sns/x_auto_poster.py     85
sns/ig_auto_poster.py    82
sns/posting_switch.py    61, 83, 86, 132
```

ただし全部を一度に移す必要はない。共有ボードに要るのは `週次_レビュー`（=`articles`）
だけなので、`publish()` と `load_recent()` の2箇所を先に移し、`X投稿キュー`/`IG投稿キュー`
と `posting_switch` のキルスイッチはシートに残す。**投稿ジョブに一切手を触れずに済む。**

### 課題・備考

- **本番 Supabase への適用が未了**。DDL には DB パスワードか supabase CLI のアクセストークンが
  要る。ローカル検証は通っているので、資格情報が手に入り次第 `supabase db push` で反映する
- モックは Tailscale 経由で iPhone に配信して確認した（`http://100.78.122.50:8787/`）
- `posts` / `post_targets` / `publish_queue` は Cloud Run 版 publisher 専用。以後は `articles` が
  投稿パイプラインの正になるが、テーブルは削除せず残す

---

## 2026-09-01（記事生成を Mac mini へ移設。24時間稼働の基盤が揃った）

### 実施内容

**投稿処理の移設状況を確認（別セッションで完了済みだった）**

康二郎さんが 24時間稼働の Mac mini（`kitano-Macmini` / Tailscale `100.78.122.50`）を導入し、
別のチャットで投稿処理の移設を完了させていた。本セッションでは実機を SSH で直接調べて照合した。

当初「MacBook と Mac mini の両方で投稿ジョブが動いており二重投稿の危険がある」と報告したが、
**これは誤りだった**。MacBook 側の plist は `.disabled` に改名済みで、`launchctl list` に
残っていたのは生成ジョブ（`weekly-loop`）のほうだった。最終実行は MacBook 13:25 →
Mac mini 13:33 で、重なりなく引き継がれていた。

**記事生成ループの移設**

生成側（`com.joyfoundation.weekly-loop`）だけが MacBook に残り、`exit=1` で失敗し続けていた。

```
Errno 8 nodename nor servname provided        ← スリープで DNS が死ぬ
API Error: went to sleep mid-response          ← 生成中にスリープ
```

Mac mini `~/agent/loop/` へ移設し、月・水・金 9:00 で launchd 登録した。MacBook 側は
`.disabled` に改名して停止。**MacBook にジョブは1つも残っていない。**

認証は `~/agent/.claude-token` を `CLAUDE_CODE_OAUTH_TOKEN` に渡す方式。`sk-ant-oat01-`
で始まる OAuth トークンなので `ANTHROPIC_API_KEY` に入れると 401 になる（実際に踏んだ）。
`.env` は既に Mac mini にあったため**新たな認証情報の転送は行っていない**。
`~/agent/sns/.env` への symlink にして置き場所を1箇所に保った。

### 成果

| | MacBook | Mac mini |
|---|---|---|
| 生成時間 | 4時間超 → 失敗 | **8分34秒**（3本・レビュー改稿込み） |
| 失敗原因 | スリープで DNS 断・生成中断 | なし |
| 画像選定 | **一度も動いていなかった** | 248枚から5枚を選定 |
| 検証結果 | — | 問題あり0本 / 問題なし3本 |

### 直した不具合

**1. 知識層カード621枚が実行環境から消えていた**

`sync_loop_runtime.sh` が `rm -rf knowledge` → `cp -R` の順で、コピー中に止まると全損する
作りだった。`_imgcache` だけ残っていたことから中断と判明。**この状態では記事が1本も作れない。**

`rsync --delete` に書き換えた。送り終えた分を見て差分を消すので、途中で切れてもカードは残る。
同期先も Mac mini に変更し、層ごとの枚数を両側で突き合わせて不一致なら終了コード1で止まる
ようにした。

**2. WebSearch の権限が無く時事文脈が取れなかった**

MacBook は `settings.json` の `defaultMode: "auto"` で全ツールが通っていたため露見して
いなかった。実行機の設定に依存させず、`collect_context.py` 側で `--allowedTools WebSearch`
を明示した（丸ごと自動承認にはしない）。

**3. キルスイッチの誤作動（三度目）**

```
根拠: …特別警報・震度6弱以上・津波警報・噴火警戒レベル4以上…は確認されなかった
⛔ キルスイッチ作動（検知語 ['噴火', '津波', '特別警報', '震度6']）
```

「無かった」と書いてある文を災害発生と読んだ。8/4・8/24 に続く同型の誤作動。語を一つずつ
足す方式では再発するため、**否定の語尾（ず・ぬ・ない・なく・なかった・ません・なし）を
一般形で捉える**ようにした。判定は節ごとなので「震度6弱が発生し、けが人はいない」では
正しく作動する。

再発に気づけなかった原因はテストが残っていなかったこと。`test_kill_switch.py` を新設し、
過去3回の誤作動文をそのまま回帰テストにした（11件・全通過）。

**4. codex 未導入でレビュー工程が丸ごと飛んでいた**

Node と `@openai/codex` 0.152.0 を導入。ChatGPT 対話ログインではなく `.env` の
`OPENAI_API_KEY` を `codex login --with-api-key` で登録した（`--api-key` フラグは廃止済み）。
**課金が ChatGPT サブスクから OpenAI API 従量課金に変わる**点は要確認。

### 課題・備考

- `--test-cards` を外した。記事IDに `TEST-` が付くと投稿候補にならず、`extract_ac.py` も
  承認の証拠から除外する。8/26 に承認対象がカードから完成投稿へ移ったため役目を終えていた
- **`git status` / `git diff` が3分以上返らない**。`git add` や `git ls-files` は動くので
  リポジトリ自体は健全。`sns-calendar-app/`（32,357ファイル）と iCloud 同期の組み合わせが
  疑わしい。`core.untrackedCache` は設定済みだが効いていない
- git の `user.name` / `user.email` が未設定のまま（ホスト名変更で消えた）
- Mac mini の `CLAUDE_CODE_OAUTH_TOKEN` は 2027年9月頃に失効する

## 2026-08-19（ヘルシートラック：先方合意内容を反映して要件・提案を改訂）

### 実施内容

**新情報の確認（NotebookLM 全11ソースを原文で直読）**

圭一郎さんから共有された NotebookLM「Driving Forest Bathing: Cultivating the Elite Driver」
（https://notebook.google.com/notebook/7615943e-8b8b-4f81-abec-6365504043c5）の全ソースを
AI要約ではなく**ソース本文**で確認した。8/3・8/17・8/18の資料が新規に入っていた。

- `8.3 議題質問内容 / Answer` — 8/3リモートのQ&A。車両・被験者・音量方針が確定していた
- `8.17 ①方向性 ②脳疲労 ③ニーナ・クラウス ⑤実験計画案 ⑥スピーカー候補`
- `8.18 ①健康習慣 ②音響・再生方法`
- `8.17 ④騒音とは何か` は書籍『音と脳』のスキャン画像でテキスト無し

**判明した主な変更点（v1.1 の前提を複数破棄）**

| 項目 | 旧（v1.1） | 新（合意済み） |
|---|---|---|
| 再生方式 | スマホ＋Bluetoothスピーカー | **TFカード／USB一体型小型スピーカー**（スマホは使わない） |
| 実験規模 | 5〜10台・10〜20名 | **3台・3名**（いすゞAT/トヨタAT/トヨタMT、女性2・男性1、全員24歳） |
| デザイン | 前半ベースライン→後半音あり | **クロスオーバー試験**（水曜は業務形態が違うため除外） |
| 測定 | POMS35項目・心拍血圧・唾液 | **POMS2・疲労VAS・眠気KSS・血圧・心拍・HRV・GHQ**（本実験で唾液コルチゾール） |
| 音量 | 記載なし | 車内騒音より**やや大きめ70dB程度**（実測前の暫定）／騒音より**高い周波数**を重ねる |
| 体感音響 | 組み合わせ実験を提案 | **本実験では組み合わせない**（研究所では既に使用中） |
| コンセプト | 「健康になる」 | **「自分の健康を育てる」→「一流のドライバーを育てる」** |

**ドキュメント改訂**

- `docs/healthy_truck_requirements_v2.md` を新規作成（v1.1を置き換え）
- `docs/healthy_truck_system_proposal_v3.md` を新規作成（v2を置き換え）。
  v2までの「方式を提案する」文書から、**合意済みの方向を実行に移す**文書へ位置づけを変更
- 旧版3本（requirements_v1 / proposal_v1 / proposal_v2）に【旧版】バナーを追加

**v3 で新規に用意したもの**

- **8/21 車内騒音測定の実施手順** — 測定位置（ドライバー耳位置）、7つの走行条件、
  WAV無圧縮録音の必須化（周波数解析のため）、記録シート項目
- **音量設計の方針転換案** — 「音量で勝つ」のではなく「**帯域で棲み分ける**」。
  騒音が低域中心なので自然音は中高域に置き、全体音量を上げずに聞こえる状態をつくる
- **スピーカー実機テスト8項目** — 特に「電源OFF→ONで自動再生されるか」「リピートの継ぎ目」は
  ワンスイッチ・常時再生という中核要件に直結するため、通らない機種は価格に関係なく不採用
- **音源パッケージの技術仕様** — ラウドネス統一、極端なダイナミクスを避ける、
  継ぎ目が自然につながる編集、連番ファイル名
- **プレ実験の運用設計** — 1日の流れ、記録シート、教示の統一（盲検化できないためのバイアス対策）

### 成果

- 要件定義 v2.0（286行）／提案書 v3（284行）
- 未解決の要確認事項を8件に整理（予算・開始日・HRV機材・測定導線・音源種類数・割付方式ほか）

### 課題・備考

- **n=3・全員24歳・同一年次**のため統計的な一般化は不可。探索的プレ実験と明記して先方と合意しておく必要がある
- 実験計画書内で**割付が固定表と「ランダム化する」で記述が矛盾**している。開始前に統一が必要
- **70dBはサイレン・警音器のマスキング**リスクがあるため、8/21実測後に必ず再評価する
- 音源の種類数が資料間で「3〜4種類」と「5〜10種類」に割れている
- 運行拠点と波動医科学研究所の距離が未確認。移動時間が長いと「運転後」の測定値が回復してしまう
